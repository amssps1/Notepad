USE [dba_db]
GO
/****** Object:  StoredProcedure [dbo].[usp_Prevencao_GrantDelete]    Script Date: 09/01/2026 14:10:14 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO

ALTER   PROCEDURE [dbo].[usp_Prevencao_GrantDelete]
(
...
BEGIN
    ...
    BEGIN TRY
        /* ============================================================
           CONFIG (HARDCODED)
           ============================================================ */
        DECLARE @Principal  sysname = N'COFIDIS2000\GRP_SQL_DADOS_MODELOS_PREV';
        ...
        VALUES
            (N'ComunicacaoParceiros'),
            (N'ControlosPermanentes'),
            (N'db_cofidis_dah'),
            (N'db_cofidis_dah_interface'),
            (N'db_cofidis_dah_interface_aux'),
            (N'db_cofidis_dah_transactionlog'),
            (N'ExtratoCofidis'),
            (N'FactCofidis'),
            (N'Hangfire'),
            (N'Interfacesdah'),
...
';
        ...
        EXEC msdb.dbo.sp_add_job
             @job_name = @JobName,
             @enabled = 1,
             @description = N'Revoke temp DELETE permission (prevenção) after 15 minutes and self-delete',
